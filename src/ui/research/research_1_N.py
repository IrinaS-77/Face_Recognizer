...
class ResearchBox(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        '''1. RUN'''
        self.bth_run_research = Button(text='Рассчитать', size_hint=(.1, .05), on_press=self.run_research, background_color = (0, 1, 0.28, .9))
        ...
        self.method_param_name = Label(size_hint=(.05, 1),color=(0,0,0,))
        self.method_param = TextInput(size_hint=(.1, 1), multiline=False, input_type='number', input_filter='int')
        self.bth_view = Button(text='Показать', size_hint=(.15, 1), on_press=self.view_CV, background_color = (0, 1, 0.28, .9))
