...
class MainBox(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            #Color(.2, .3, .5, )
            self.bg = Rectangle(source = os.path.join(DATA_DIR, 'bg.png'), pos = self.pos, size = self.size)
        ...
        self.bth_to_menu = Button(text='Меню', on_press=self.open_menu,
            size_hint = (.1, .05), background_color = (0.17, 0.17, 0.17, .9), disabled=True)

        ''' ============== Menu of main boxes ============== '''        
        self.list_submainboxes = RelativeLayout()
        bth_classifier_mainbox = Button(text ="Классификатор", on_press=self.open_submain,
            size_hint = (.2, .05), pos_hint={"center_x":.294, "center_y":.60}, background_color = (0.17, 0.17, 0.17, .9))
        bth_research_mainbox = Button(text ="Исследования", on_press=self.open_submain,
            size_hint = (.2, .05), pos_hint={"center_x":.294, "center_y":.50}, background_color = (0.17, 0.17, 0.17, .9))
        bth_parallel_system_mainbox = Button(text ="Параллельная система", on_press=self.open_submain,
            size_hint = (.2, .05), pos_hint={"center_x":.294, "center_y":.40}, background_color = (0.17, 0.17, 0.17, .9))
