...
class MainBox(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.list_databases = Button(text='Базы', size_hint=(.5, 1), background_color=(0.17, 0.17, 0.17, .9))
        ...
        ''' 1.2 RUN ================================ '''
        self.bth_run_research = Button(text='Рассчитать', size_hint=(.4, 1), on_press=self.run_research,
                                       background_color=(0, 1, 0.28, .9))

        self.box_L = BoxLayout(orientation='horizontal', size_hint=(.1, 1), spacing=1)
        L_name = Label(text="L", color=(0, 0, 0,), size_hint=(.4, 1))
        self.L = TextInput(
            text='5',
        ...
        self.research_box = BoxLayout(orientation='vertical', size_hint=(.75, .95), pos_hint={"center_x": .5},
                                      spacing=10)
        ...
        for method in ALL_METHODS:
            ...
            method_name = Label(text=f"{method}({METHODS_PARAM[method]['name']}):", color=(0.01, 0.71, 0.21, .9),
                                size_hint=(.4, 1))
        ...
        plt.title('Результат параллельной системы L/N-L', fontsize=self.fs)
    ...
    def run_research(self, instance):
        ...
        x = [f'{im}/{number_img - im}\n{im * number_classes}/{(number_img - im) * number_classes}' for im in
           range(1, number_img)]
